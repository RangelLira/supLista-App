# Textos da ficha da Play Store — supLista

## Descrição curta (máx. 80 caracteres)

Listas de compras e tarefas numa lista só. Sem anúncios, com compartilhamento.

## Descrição completa (máx. 4000 caracteres)

supLista é um app de listas de compras e tarefas — tudo numa lista só. Rápido, sem cadastro obrigatório, sem anúncios, sem rastreamento.

UMA LISTA PARA TUDO
Numa mesma lista, misture itens de mercado (com quantidade, unidade e preço, acompanhando o total gasto) e itens de tarefa simples (só o nome, sem quantidade). Não precisa escolher um tipo de lista — organize do seu jeito.

COMPARTILHAMENTO EM TEMPO REAL (OPCIONAL)
Conecte-se a outra pessoa por um código de convite e compartilhe uma lista. As alterações de qualquer lado aparecem na hora nos dois aparelhos, perfeito para a lista do mercado da família. O compartilhamento usa a sua conta Google apenas para identificar você; se não quiser, o app funciona 100% offline no seu aparelho.

ARQUIVAMENTO
Concluiu uma lista? Arquive para manter o histórico organizado sem poluir a tela principal, e recupere quando quiser.

RECURSOS
- Uma lista só, para compras e tarefas
- Preço por item, com total automático
- Anotações por lista
- Arquivamento de listas concluídas
- Tema claro, escuro ou automático, com 4 cores de destaque
- Português, inglês e espanhol
- Compartilhamento em tempo real: o dono controla exclusão e fim do compartilhamento
- Funciona offline

PRIVACIDADE EM PRIMEIRO LUGAR
Suas listas ficam no seu aparelho. Nada é enviado para a internet, exceto as listas que você escolher compartilhar. Não há anúncios, telemetria nem venda de dados.
Política de privacidade: https://rangellira.github.io/supLista-App/privacy.html

GRATUITO E DE CÓDIGO ABERTO
O supLista é gratuito e o código-fonte é público: https://github.com/RangelLira/supLista-App

Desenvolvido por Contest Software.

---

## EN — Short description

Shopping and task lists in one. Ad-free, with real-time sharing.

## EN — Full description

supLista is a shopping and task list app — everything in one list. Fast, no mandatory sign-up, no ads, no tracking.

ONE LIST FOR EVERYTHING
Mix grocery items (with quantity, unit and price, tracking the running total) and simple task items (just a name, no quantity) in the same list. No need to pick a list type — organize it your way.

REAL-TIME SHARING (OPTIONAL)
Connect with another person via an invite code and share a list. Changes from either side show up instantly on both phones, perfect for the family grocery list. Sharing uses your Google account only to identify you; if you'd rather not, the app works 100% offline on your device.

ARCHIVING
Finished a list? Archive it to keep your history tidy without cluttering the main screen, and bring it back whenever you want.

FEATURES
- One list type, for groceries and tasks
- Price per item, with an automatic total
- Per-list notes
- Archiving for finished lists
- Light, dark or automatic theme, with 4 accent colors
- Portuguese, English and Spanish
- Real-time sharing: the owner controls deletion and ending the share
- Works offline

PRIVACY FIRST
Your lists stay on your device. Nothing is sent to the internet except the lists you choose to share. No ads, no telemetry, no data selling.
Privacy policy: https://rangellira.github.io/supLista-App/privacy.html

FREE AND OPEN SOURCE
supLista is free and its source code is public: https://github.com/RangelLira/supLista-App

Made by Contest Software.

---

## ES — Short description

Listas de compras y tareas en una sola. Sin anuncios, con uso compartido.

## ES — Full description

supLista es una app de listas de compras y tareas: todo en una sola lista. Rápida, sin registro obligatorio, sin anuncios, sin rastreo.

UNA LISTA PARA TODO
En la misma lista, combina artículos de supermercado (con cantidad, unidad y precio, siguiendo el total gastado) y tareas simples (solo el nombre, sin cantidad). No hace falta elegir un tipo de lista: organízala como quieras.

USO COMPARTIDO EN TIEMPO REAL (OPCIONAL)
Conéctate con otra persona mediante un código de invitación y comparte una lista. Los cambios de cualquier lado aparecen al instante en ambos teléfonos, perfecto para la lista del supermercado de la familia. El uso compartido usa tu cuenta de Google solo para identificarte; si prefieres no hacerlo, la app funciona 100% sin conexión en tu dispositivo.

ARCHIVADO
¿Terminaste una lista? Archívala para mantener el historial ordenado sin llenar la pantalla principal, y recupérala cuando quieras.

FUNCIONES
- Una sola lista, para compras y tareas
- Precio por artículo, con total automático
- Notas por lista
- Archivado de listas terminadas
- Tema claro, oscuro o automático, con 4 colores de acento
- Portugués, inglés y español
- Uso compartido en tiempo real: el propietario controla la eliminación y el fin del compartido
- Funciona sin conexión

LA PRIVACIDAD PRIMERO
Tus listas quedan en tu dispositivo. No se envía nada a internet, salvo las listas que elijas compartir. Sin anuncios, sin telemetría, sin venta de datos.
Política de privacidad: https://rangellira.github.io/supLista-App/privacy.html

GRATIS Y DE CÓDIGO ABIERTO
supLista es gratis y su código fuente es público: https://github.com/RangelLira/supLista-App

Hecho por Contest Software.
