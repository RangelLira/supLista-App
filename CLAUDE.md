...
### State Management
There is no external state library. `App.tsx` is the single source of truth — it holds the full `lists` array in `useState` and passes it down as props. Mutation handlers in `App.tsx` (`handleSaveList`/`handleUpdateList`/`handleDeleteList`/`handleArchiveList`/`handleUnarchiveList`) only call `setLists`; a **single `useEffect(() => saveLists(lists), [lists])`** persists to AsyncStorage (never call `saveLists` from inside a `setState` updater — the updater must stay pure). There is no item catalog anymore — "Buscar itens" / "Herdar itens" read the user's own lists directly (active + archived).

### Data Model (`src/types/index.ts`)
- **`ShoppingList`** — the only content entity. **No `type` field** — there is no shopping-vs-task list distinction; one list mixes both. A list is `isArchived` (goes to the Archive, hidden from the main screen, `archivedAt` set) or not. `notes?: string` is free-text.
- **`ListItem`** — quantity, unit, checked state, optional price (`priceType: 'unit' | 'total'`). Price is set/edited inline in `AddItemModal` (no separate price dialog). **`unit: 'tarefa'`** (`TASK_UNIT`) marks the item as a *task*: it renders name-only (no qty, no price fields). Any list can mix task items and priced items.

### Screens (`src/screens/`)
Navigation is manual — `App.tsx` maintains `activeScreen: ScreenName` (`'listas' | 'config' | 'arquivo'`) + a `screenHistory` stack, and renders the matching screen. `'arquivo'` (`ArchiveScreen`) is reached from the Settings menu ("Arquivo de Listas", listed after Preferências); it shows archived lists as non-openable cards — swipe → recovers (back to main as completed), swipe ← deletes permanently. There is no navigation library and **no bottom tab bar** — this app has one primary screen (Listas), so navigation to Settings is a single hamburger button (see below).
...
### Key Flows
- **Startup**: `init()` in `App.tsx` loads settings + lists and calls `purgeLegacyCatalog()` (drops the removed `@suplista_item_catalog` key). No auto-cleanup — **deleting a list is permanent and wipes its items** (nothing is retained anywhere). Archiving is the "keep for reuse" path.
...
- **Completing a list**: `handleUpdateList` in `App.tsx` is the single place that stamps/clears `completedAt` (on the `isCompleted` flip). The screens only flip the boolean. Completing a list with unchecked items shows a "Concluir mesmo assim?" confirm (button **and** swipe).
- **Archiving**: only a **completed** list you **own** can be archived (button in the list-detail bottom bar where "Adicionar item" was; guests never see it). `handleArchiveList` sets `isArchived`/`archivedAt` and nullifies `sharedWithUid` (ending the share). `handleUnarchiveList` clears both and marks the list completed. Archived lists are hidden from the main screen but still feed "Herdar itens" / "Buscar itens".
- **Adicionar item** (`AddItemModal`, "Adicionar item na lista:"): name + qty/unit + optional **price** inline. If unit is "Tarefa", qty and price fields hide. The `<Modal>` overlay is **top-anchored** (`justifyContent:'flex-start'`), not centered — a centered card sits behind the keyboard that the name field's autofocus raises. Two extra buttons (add mode only): **Herdar itens** and **Buscar itens** — both open full-screen conditional renders (`InheritItemsView` / `SearchItemsView`, driven by `subView` state, NOT `<Modal>`), multi-select, sourced from the user's own lists (active + archived), never lists shared *to* the user.
...
- **devLog** (`src/dev/devLog.ts`): ring buffer (5000 lines) persisted to `@suplista_devlog`, plus a `console.warn`/`console.error` hook. `initDevLog()` is called once from `App.tsx` `init()`; `logEvent(tag, msg, data?)` is a no-op outside `__DEV__` and is sprinkled through `App.tsx` handlers (`handleSaveList`/`handleUpdateList`/`handleDeleteList`/`syncSharedList`/listeners/nav). Log tab: copy last 1000 lines / share / clear.
- **fakeData** (`src/dev/fakeData.ts` + `pools.ts`): `generateFakeData({seed})` builds ~280 lists / ~4300 items (1 list of 1000, ~9 lists >100, rest ≤10), preset + custom tags, task+priced items, some archived, no sharing. Deterministic per seed. Returns `{ lists, report }`; the report is a plain-text summary shown in DevPanel (copy / share). "Limpar tudo" wipes `@suplista_lists` (keeps settings/onboarding). After Fake user / Limpar tudo, `DevSettings.reload()` restarts the JS.
...
### Persistence
AsyncStorage keys: `@suplista_lists` (lists), `@suplista_settings` (settings), `@suplista_devlog` (dev log, `__DEV__` only). All reads/writes go through `src/utils/storage.ts` (except the dev log — `src/dev/devLog.ts`).
