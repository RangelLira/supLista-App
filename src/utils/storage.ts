...
// Chave do antigo catálogo de itens — removido. Limpa uma vez no boot para não
// deixar lixo no armazenamento de quem já usou a versão com catálogo.
export const purgeLegacyCatalog = async (): Promise<void> => {
  try { await AsyncStorage.removeItem('@suplista_item_catalog'); } catch { /* ignore */ }
};
